...
def create_folder_if_not_exists(folder_path):
    try:
        ...
        # An exception will be caught if there's an issue creating the directory, other than it already existing.
...
def main(args):
    ...
    for path in data_paths:
        ...
        if args.ce_model is not None:
            ...
            reranked_results = ranker.rerank(passage_id_map, data, results, top_k=args.n_docs, prompt=args.prompt)

            filename = os.path.basename(args.data)

            if re.search('\.jsonl$', filename):
                # Generate the new file name by replacing .jsonl with .pickle
                filename = re.sub('\.jsonl$', '.pickle', filename)
            
            create_folder_if_not_exists(f"./reranked_results/{args.dataset}")
            with open(f"./reranked_results/{args.dataset}/result_{filename}", "wb") as f:
                pickle.dump(reranked_results, f)

            print(f"finish running {args.dataset} {filename}")
            return
